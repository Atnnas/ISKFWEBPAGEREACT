...
export default function WrittenExamsView({ initialExams = [] }) {
  ...
  // Escala de tamaño tipográfico accesible (A / A+ / A++) con persistencia
  const [adminFontScale, setAdminFontScale] = useState('normal'); // 'normal' | 'large' | 'xlarge'

  useEffect(() => {
    if (typeof window !== 'undefined') {
      const savedScale = localStorage.getItem('iskf_admin_exam_font_scale');
      if (savedScale && ['normal', 'large', 'xlarge'].includes(savedScale)) {
        setAdminFontScale(savedScale);
      }
    }
  }, []);
  ...
  return (
    <div className={`max-w-4xl mx-auto pt-2 pb-16 space-y-8 exam-text-scale-${adminFontScale}`}>
      ...
      {currentView === 'list' && (
        <div className="space-y-6">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-gray-200/80 pb-5">
            <div>
              <div className="flex items-center gap-2 text-xs sm:text-sm text-gray-600 font-semibold mb-1.5">
              ...
              <p className="text-gray-700 text-xs sm:text-sm md:text-base font-medium mt-0.5">
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              <button
                ...
                className="flex items-center justify-center gap-2 px-5 py-2.5 bg-gradient-to-r from-[#2D2E83] to-[#be1322] hover:from-[#232468] hover:to-[#9c0f1b] text-white rounded-xl text-xs sm:text-sm font-bold transition-all shadow-md shadow-[#2D2E83]/20 active:scale-95 cursor-pointer"
              >
        ...
        <div className="space-y-6">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-gray-200/80 pb-5">
            ...
            <div className="flex flex-wrap items-center gap-2.5">
              ...
              <button
                ...
                className="flex items-center justify-center gap-2 px-3.5 py-2 bg-emerald-50 hover:bg-emerald-100 text-emerald-700 border border-emerald-300 rounded-xl text-xs sm:text-sm font-bold transition-all shadow-sm active:scale-95 cursor-pointer"
              ...
              <button
                ...
                className="flex items-center justify-center gap-2 px-4 py-2 bg-gradient-to-r from-[#2D2E83] to-[#be1322] hover:from-[#232468] hover:to-[#9c0f1b] text-white rounded-xl text-xs sm:text-sm font-bold transition-all shadow-md active:scale-95 cursor-pointer"
              >
                <Plus className="w-4 h-4" />
          ...
          <div className="space-y-4">
            {(activeExam.questions || []).map((q, index) => (
              <div
                key={q.id}
                className="bg-white/95 border border-gray-200/90 rounded-2xl p-5 sm:p-6 space-y-3.5 transition-all shadow-sm text-gray-900"
              >
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2.5">
                    <span className="w-8 h-8 rounded-xl bg-blue-50 border border-blue-200 flex items-center justify-center text-sm font-black text-[#2D2E83]">
                      {index + 1}
                    </span>
                    {getTypeBadge(q.type)}
                    <span className="text-xs font-bold px-2.5 py-1 rounded-lg bg-gray-100 border border-gray-200 text-gray-700 font-mono">
                  ...
                  <div className="flex items-center gap-1">
                    <button
                      ...
                      className="p-2 text-gray-600 hover:text-[#2D2E83] hover:bg-blue-50 rounded-xl transition-colors cursor-pointer"
                    ...
                    <button
                      ...
                      className="p-2 text-gray-400 hover:text-[#BE1622] hover:bg-red-50 rounded-xl transition-colors cursor-pointer"
                      title="Quitar pregunta"
                    >
                      <Trash2 className="w-4 h-4" />
                    </button>
                ...
                <p className="text-base sm:text-lg font-black text-gray-900 leading-relaxed pl-1">
                  {q.text}
                </p>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 pt-1 pl-1">
                    {q.options.map((opt, optIndex) => {
                      ...
                      return (
                        <div
                          key={optIndex}
                          className={`flex items-center justify-between px-3.5 py-2.5 rounded-xl text-xs sm:text-sm border ${
                            isCorrect
                              ? 'bg-emerald-50 border-emerald-300 text-emerald-900 font-bold'
                              : 'bg-gray-50/80 border-gray-200 text-gray-900 font-medium'
                          }`}
                        >
                          <div className="flex items-center gap-2.5 truncate pr-2">
                            <span className={`w-6 h-6 rounded-lg flex items-center justify-center text-xs font-black shrink-0 ${
                              isCorrect ? 'bg-emerald-200 text-emerald-900' : 'bg-gray-200 text-gray-800'
                            }`}>
                ...
                {q.type === 'short_answer' && (
                  <div className="pt-1 pl-1">
                    <div className="bg-amber-50/70 border border-amber-200 rounded-xl p-3.5 text-xs sm:text-sm text-amber-950 flex flex-col sm:flex-row sm:items-center justify-between gap-2 font-medium">
                      <span>Formato: Respuesta corta (1 a 2 líneas de texto por parte del alumno).</span>
                      <span className="font-black font-mono bg-white px-3 py-1 rounded-lg border border-amber-300 text-amber-900 shadow-2xs self-start sm:self-auto text-xs">
                ...
                {q.type === 'long_answer' && (
                  <div className="pt-1 pl-1">
                    <div className="bg-purple-50/70 border border-purple-200 rounded-xl p-3.5 text-xs sm:text-sm text-purple-950 flex flex-col sm:flex-row sm:items-center justify-between gap-2 font-medium">
                      <span>Formato: Respuesta larga / desarrollo teórico libre.</span>
                      <span className="font-black font-mono bg-white px-3 py-1 rounded-lg border border-purple-300 text-purple-900 shadow-2xs self-start sm:self-auto text-xs">
                        Valor: {getQuestionPointValue(q)} pt{getQuestionPointValue(q) === 1 ? '' : 's'} (se suma al calificar)
                      </span>
                    </div>
                  </div>
                ...
                {q.type === 'matching' && q.leftTerms && q.topTerms && (
                  <div className="pt-2 pl-1 overflow-x-auto">
                    <table className="min-w-full border-collapse text-xs sm:text-sm border border-gray-200 rounded-xl overflow-hidden shadow-2xs">
                      <thead>
                        <tr className="bg-gray-100/80">
                          <th className="p-3 border-b border-r border-gray-200 text-gray-800 font-black text-left">
                            Eje Izquierdo \ Eje Superior
                          </th>
                          {q.topTerms.map((col, cIdx) => (
                            <th key={cIdx} className="p-3 border-b border-gray-200 text-[#2D2E83] font-black text-center whitespace-nowrap">
                              {col}
                            </th>
                          ))}
                        </tr>
                      ...
                      <tbody>
                        {q.leftTerms.map((row, rIdx) => {
                          ...
                          return (
                            <tr key={rIdx} className="border-b border-gray-100 hover:bg-blue-50/20 font-medium">
                              <td className="p-3 border-r border-gray-200 font-bold text-gray-900 bg-gray-50/60">
                                {row}
                              </td>
                              {q.topTerms.map((_, cIdx) => {
                                const isMatched = matchedCol === cIdx;
                                return (
                                  <td key={cIdx} className="p-3 border-gray-100 text-center">
                                    {isMatched ? (
                                      <span className="inline-flex items-center justify-center w-7 h-7 rounded-full bg-emerald-100 text-emerald-800 border border-emerald-300 mx-auto shadow-xs font-bold" title="Asociación correcta">
                                        <Check className="w-4 h-4" />
                                      </span>
                                    ) : (
                                      <span className="inline-block w-2.5 h-2.5 rounded-full bg-gray-300 mx-auto"></span>
                                    )}
                                  </td>
                                );
                              })}
